...
const trendingReviews = [...mockReviews].sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0));
const topLikedReviews = [...mockReviews].sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0)).slice(0, 5);

// Simulate "From Friends" - for now, just picks a few reviews from other users
const reviewsFromFriends = mockReviews.filter(r => r.userId !== mockUsers[0].id).slice(0,5); 

// Simulate "Recommended" - for now, just shuffles some reviews
const recommendedReviews = [...mockReviews].sort(() => 0.5 - Math.random()).slice(0,10);
...
const actionGenreReviews = mockReviews.filter(r => r.genres.includes('Action')).sort((a,b) => b.createdAt.getTime() - a.createdAt.getTime()).slice(0,5);
const sciFiGenreReviews = mockReviews.filter(r => r.genres.includes('Sci-Fi')).sort((a,b) => b.createdAt.getTime() - a.createdAt.getTime()).slice(0,5);
...
export default function HomePage() {
  return (
    <div className="space-y-12">
      ...
      <ContentCarousel title="From People You Might Know" items={reviewsFromFriends} />

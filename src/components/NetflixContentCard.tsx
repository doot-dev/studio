  ...
  return (
    <Card className={cn("overflow-hidden group relative flex flex-col h-full shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out bg-card border-transparent hover:border-primary/50 transform hover:-translate-y-1", className)}>
      <Link href={`/review/${review.id}`} passHref legacyBehavior>
        <a className="block focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 rounded-t-lg" aria-label={`View details for ${review.movieTitle}`}>
      ...
      <div className="p-2 text-center flex-grow flex flex-col justify-between">
        ...
        {/* Genre tags can be re-added here if desired for mobile, but keeping it minimal for now */}
        {/* <div className="flex flex-wrap gap-1 justify-center mt-1">
          {review.genres.slice(0, 1).map((genre) => ( // Show only one genre tag for brevity
            <Badge key={genre} variant="secondary" className="text-xs bg-white/10 text-white/80 backdrop-blur-sm px-1.5 py-0.5">
              {genre}
            </Badge>
          ))}
        </div> */}
